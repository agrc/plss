...
{
    public class HashPasswordCommandAsync : CommandAsync<PasswordHashAndSalt>
    {
        public HashPasswordCommandAsync(string passwordToHash, string pepper)
        {
            Pepper = pepper;
        ...
        public HashPasswordCommandAsync(string passwordToHash, string salt, string pepper)
        {
            Pepper = pepper;
            PasswordToHash = passwordToHash + salt;
            PasswordInfo = new PasswordHashAndSalt(PasswordToHash, salt);
        ...
        public override async Task<PasswordHashAndSalt> Execute()
        {
            return await Task<PasswordHashAndSalt>.Factory.StartNew(() =>
                {
