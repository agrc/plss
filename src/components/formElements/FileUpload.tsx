...
const validateSize = (actualSize: number, megabytes: number) => {
  return actualSize > megabytes * 1024 * 1024;
};

const FileUpload = ({ id, defaultFileName, path, contentTypes, maxFileSize, value, onChange }: FileUploadProps) => {
  const { storage } = useFirebaseStorage();
  const uploadReference = useRef<StorageReference | undefined>(undefined);
  ...
  const uploadFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const [hasValidType, errorMessage] = validateContentTypes(fileToUpload.type, contentTypes);
    if (!hasValidType) {
    ...
    uploadTask?.on(
      ...
      async () => {
        const uploadedReference = uploadReference.current;
        if (!uploadedReference) {
          return;
        }

        onChange?.(uploadedReference.fullPath);
        setUploadPercent(undefined);
...
export const ObjectPreview = ({ url, children }: ObjectPreviewProps) => {
  return (
...
export const Attachment = ({ onClick }: AttachmentProps) => (
