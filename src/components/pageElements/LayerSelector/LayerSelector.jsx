...
const ExpandableContainer = ({ expanded, children }) => {
  const [open, setOpen] = useState(expanded);

  const imageClasses = classNames('layer-selector__toggle', {
    'layer-selector--hidden': open,
  });

  const fromClasses = classNames({ 'layer-selector--hidden': !open });
  ...
  return (
    ...
    >
      ...
      <form className={fromClasses}>{children}</form>
    </div>
  );
};
